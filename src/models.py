...
class BevEncode(nn.Module): # 对得到的BEV特征进行特征提取并得到最终的分割结果
    ...
    def forward(self, x):  # x: 4 x 64 x 200 x 200  BEV特征
...
class LiftSplatShoot(nn.Module):
    ...
    def create_frustum(self): # 对图像的每个像素点生成深度集合D的坐标, 这里的像素实际上是原图像(128*352)经过16倍下采样后的像素点
        ...
        # 返回一个一维的tensor, 这个张量包含了从start到end(包括端点)的等距的steps个数据点
    ...
    def get_geometry(self, rots, trans, intrins, post_rots, post_trans):
        ...
        # B x N x D x H x W x 3  points
        # 抵消数据增强及预处理对像素的变化, 输入的self.frustum的像素点是数据增强(经过增强的旋转平移处理)后的点 即: self.frustum  = R*points+T, 所以为了还原points, 需要进行处理
        # post_rots: 4 x 5 x 3 x 3
        # post_trans: 4(B) x 5(N) x 3(平移向量) post_trans.view(B, N, 1, 1, 1, 3)-->B x N x D x H x W x 3
        points = self.frustum - post_trans.view(B, N, 1, 1, 1, 3)
        points = torch.inverse(post_rots).view(B, N, 1, 1, 1, 3, 3).matmul(points.unsqueeze(-1))

        # cam_to_ego 将points的第三维lamda乘进去  lamda(u,v,1)
        points = torch.cat((points[:, :, :, :, :, :2] * points[:, :, :, :, :, 2:3],
                            points[:, :, :, :, :, 2:3]
                            ), 5)  # 将像素坐标(u,v,d)变成齐次坐标(du,dv,d)
        # 经过上一句代码, 此时像素坐标的lamda已经乘进像素坐标中,并且已知, [du,dv,d]^T=intrins*rots^(-1)*([x,y,z]^T-trans), 根据上述公式反求出[x,y,z]  在nuscenes数据集中比较特殊, ego坐标系的旋转平移向量是相机-->ego的, 即相机坐标系坐标为X, ego为Y的话, Y=RX+T, 因此, ego-->相机的话, 公式为X = R^(-1)*(Y-T), 然后再用内参去将X转换为像素坐标
        # 坐标系转换: https://blog.csdn.net/lyhbkz/article/details/82254069  nuscenes数据集的坐标系转换 https://blog.csdn.net/weixin_44398263/article/details/120063785
        # 根据式[du,dv,d]^T=intrins*rots^(-1)*([x,y,z]^T-trans), ego坐标系中的点云的坐标[X,Y,Z] = R*K^(-1)*[du,dv,d]+T, 这是由于ego坐标系到相机坐标系的特殊转换导致的, 因此, 在不同数据集下需要看好给的旋转平移向量是从哪个坐标系到哪个坐标系
        combine = rots.matmul(torch.inverse(intrins))
        points = combine.view(B, N, 1, 1, 1, 3, 3).matmul(points).squeeze(-1)
        points += trans.view(B, N, 1, 1, 1, 3)  # 将像素坐标d[u,v,1]^T转换到车体坐标系下的[x,y,z]^T

        return points  # B x N x D x H x W x 3 (4 x 6 x 41 x 8 x 22 x 3)  points是自车坐标系下的生成的点
    ...
    def get_cam_feats(self, x):
        ...
        x = self.camencode(x) # 进行图像编码  x: B*N x C x D x fH x fW(24 x 64 x 41 x 8 x 22) 在self.camencode进行了16倍的下采样, 使得imH: 128  imW: 352-->8*22
    ...
    def voxel_pooling(self, geom_feats, x):
        ...
        # bx=[-49.75,-49.75,0]
        # nx=[200,200,1]
        # flatten indices  (self.bx - self.dx/2.) = [-50.0, -50.0, -10], 使用生成的点云坐标减去点云范围初始边界, 然后在除以xyz的网格大小, 得到每个点的网格坐标
        ...
        # get tensors from the same voxel next to each other  (x y z batch_id)  x*200*1*B + y*1*B + z*B + B
    ...
    def get_voxels(self, x, rots, trans, intrins, post_rots, post_trans):
        geom = self.get_geometry(rots, trans, intrins, post_rots, post_trans)  # 像素坐标到自车中坐标的映射关系 geom: B x N x D x H x W x 3 (4 x 6 x 41 x 8 x 22 x 3) 3表示生成点的坐标(已经转化到自车坐标系)
        x = self.get_cam_feats(x)  # 提取图像特征并预测深度编码 x: B x N x D x fH x fW x C(4 x 6 x 41 x 8 x 22 x 64)  64表示生成点的语义特征
        # gemo是将原图像尺寸缩小16倍后, 然后根据像素点以及每个像素点的D再映射到自车中坐标中的生成点云, x是根据原图像下采样16倍生成的特征, 并且是每个深度都具有的特征 
        x = self.voxel_pooling(geom, x)  # x: 4 x 64 x 200 x 200  gemo指出了生成点的坐标, x指出了生成点的特征
