...
class NuscData(torch.utils.data.Dataset):
    ...
    def get_image_data(self, rec, cams): # 获取img的同时, 还进行了图像的增强以及相机内外参的收集
        imgs = []
        rots = []
        trans = []
        intrins = []
        ...
        for cam in cams:
            ...
            # 将进行数据增强的旋转平移矩阵写成3维矩阵的形式
            post_tran = torch.zeros(3)
            post_rot = torch.eye(3)
            post_tran[:2] = post_tran2
            post_rot[:2, :2] = post_rot2
            # 将不同摄像头下的图像(进行数据增强)、内参、外参、数据增强的内外参分别放在列表中
    ...
    def get_binimg(self, rec):
        ...
        return torch.Tensor(img).unsqueeze(0)  # 转化为Tensor 1x200x200  这里应该是相当于标签
...
class SegmentationData(NuscData):
    ...
    def __getitem__(self, index):
        ...
        # imgs: 6,3,128,352  图像数据, 原图像1600*900, 但是self.get_image_data已经有数据增强
        # rots: 6,3,3  相机坐标系到自车坐标系的旋转矩阵
        # trans: 6,3  相机坐标系到自车坐标系的平移向量
        # intrins: 6,3,3  相机内参
        # post_rots: 6,3,3  数据增强的像素坐标旋转映射关系
        # post_trans: 6,3  数据增强的像素坐标平移映射关系
        binimg = self.get_binimg(rec)  # 得到rec中所有box相对于自车坐标系(ego)的box坐标的平面投影图, 1x200x200
...
def compile_data(version, dataroot, data_aug_conf, grid_conf, bsz,
    ...
    }[parser_name]  # 根据传入的参数选择数据解析器, 在train.py中, parser_name是'segmentationdata', 相当于parser为SegmentationData, 然后给parser传入参数nusc, is_train=True....
